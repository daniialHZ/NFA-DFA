...
class Transition {
  // این کلاس اصلی ترین بخش کد هست که موقعیت فعلی ، بعد و سیمبل حرکت رو مقدار دهی میکنه و در دام.جی-اس با این کلاس سر و کار داریم
...
function generateDFA(nfa, step_counter_stop = -1) {
  ...
  // Check that the nfa has any lambda Stata ?
  ...
  while (stack.length > 0) {
    ...
    for (let i = 0; i < nfa.alphabet.length; i++) {
      ...
      if (combinedStatesUnion != null) {
        ...
        if (!dfa_states.includes("تله")) {
          for (let n = 0; n < nfa.alphabet.length; n++)
            dfa_transitions.push(
              new Transition("تله", ["تله"], nfa.alphabet[n])
            );

          dfa_states.push("تله");
        }

        dfa_transitions.push(new Transition(state, ["تله"], nfa.alphabet[i]));
      }
    }
  }
